...
function widget:GetInfo()
	return {
		name = "Blueprint",
		desc = "Saves and queues groups of unit blueprints, sus",
...
---@type Blueprint[]
local quickIndex_blueprintTable = {}

local hasForcedBlueprintToBeEnabled = false
local state = {
	---@type Point|nil
	...
	---@type number
	---if this is zero, then no quick index was selected in the command notifiy call|
...
local function setSelectedBlueprintIndex(index)
	...
	local quickIndex = indexOf(quickIndex_blueprintTable, blueprint)
	if blueprintPlacementActive and index ~= nil and index > 0 then
		Spring.Echo(string.format("[Blueprint] selected blueprint #%d, quick index found #%d", selectedBlueprintIndex, quickIndex))
	end
end
...
local function setBlueprintHotkey(index, quickIndex)
	...
	local blueprint = blueprints[index]
	blueprint.quickIndex = quickIndex

	local oldBlueprint = quickIndex_blueprintTable[quickIndex]
	if oldBlueprint then 
		oldBlueprint.quickIndex = 0
	end
	quickIndex_blueprintTable[quickIndex] = blueprint
end
...
local function getBlueprintByQuickIndex(quickIndex)
	return quickIndex_blueprintTable[quickIndex]
end
-- callins
-- =======
...
function widget:Update(dt)
	...
	local quickBlueprint = getBlueprintByQuickIndex(state.selectedQuickIndex)
	local blueprint = getSelectedBlueprint()
	local foundQuickBlueprint = state.selectedQuickIndex and state.selectedQuickIndex > 0 and quickBlueprint
	if foundQuickBlueprint then
		blueprint = quickBlueprint
		--if state.selectedQuickIndex is not zero, that is efectively the same as giving the CMD_BLUEPRINT_PLACE
		if not blueprintPlacementActive then
			cmdID = CMD_BLUEPRINT_PLACE
		end
		setSelectedBlueprintIndex(indexOf(blueprints, blueprint))
		--state.blueprint = blueprint
		state.selectedQuickIndex = 0
		Spring.Echo(string.format("update blueprint detected, cmdID [%s] prevcmdID [%s]foundQuickBlueprint [%s]", tostring(cmdID), tostring(prevActiveCommand), tostring(foundQuickBlueprint)))
		hasForcedBlueprintToBeEnabled = true
		local commandIndex = Spring.GetCmdDescIndex(CMD_BLUEPRINT_PLACE)
		Spring.SetActiveCommand(commandIndex, 1, false, false, Spring.GetModKeyState())
		-- setBlueprintPlacementActive(true)
	end
	
	local eval = false
		if cmdID == CMD_BLUEPRINT_PLACE then
			eval = true
		end
		-- if foundQuickBlueprint then
		-- 	eval = true
		-- end
	if cmdID ~= prevActiveCommand and not hasForcedBlueprintToBeEnabled then
		Spring.Echo(string.format("changeing state, cmdID [%s] eval [%s] prevcmdID [%s]foundQuickBlueprint [%s]", tostring(cmdID), tostring(eval), tostring(prevActiveCommand), tostring(foundQuickBlueprint)))
		setBlueprintPlacementActive(eval)
		prevActiveCommand = cmdID
	end
	hasForcedBlueprintToBeEnabled = false
...
local drawCursorText = glListCache(function(index)
	...
	if quickIndex > 0 then
		text = text..string.format("control group: %d",quickIndex)
	else
		controlGroupText = text.."control group: none"
	end

	gl.Text(text, 15, -12, 40, "ao")
	local controlGroupText = ""
...
local function handleBlueprintSetHotkey(_, _, args)
	local quickIndex = args and tonumber(args[1])
	if not blueprintPlacementActive then
		return
	end

	if #blueprints == 0 then
		Spring.Echo("[Blueprint] no blueprints to set hotkeys")
		return
	end
	...
	setBlueprintHotkey(selectedBlueprintIndex, quickIndex)
	Spring.Echo(string.format("[Blueprint] assigned hotkey to blueprint index (%d) to quickIndex (%d)", selectedBlueprintIndex, quickIndex))
	Spring.PlaySoundFile(sounds.deleteBlueprint, 0.75, "ui")

	return true
end

local function handleBlueprintSelectHotkey(_, _, args)
	local quickIndex = args and tonumber(args[1])
	local blueprintAtQuickIndex = getBlueprintByQuickIndex(quickIndex)

	if blueprintAtQuickIndex then
		if not isValidBlueprint(blueprintAtQuickIndex) then
			Spring.Echo("[Blueprint] invalid blueprint at quick index " .. quickIndex)
		else
			state.selectedQuickIndex = quickIndex
			
			--setSelectedBlueprintIndex(quickIndex)
			Spring.PlaySoundFile(sounds.selectBlueprint, 0.75, "ui")
			return true
		end
	else
		Spring.Echo("[Blueprint] no blueprint at quick index " .. quickIndex)
	end
end
...
local function serializeBlueprint(blueprint)
	...
	return {
		...
		quickIndex = indexOf(quickIndex_blueprintTable, blueprint),
...
local function loadBlueprintsFromFile()
	...
	for index, blueprint in ipairs(blueprints) do
		if blueprint.quickIndex == nil then
			blueprint.quickIndex = 0
		end
		if blueprint.quickIndex > 0 then
			quickIndex_blueprintTable[blueprint.quickIndex] = blueprint
		end
	end
...
function widget:Initialize()
	...
	widgetHandler.actionHandler:AddAction(self, "blueprint_select_hotkey", handleBlueprintSelectHotkey, nil, "p")
	widgetHandler.actionHandler:AddAction(self, "blueprint_set_hotkey", handleBlueprintSetHotkey, nil, "p")
...
function widget:Shutdown()
	...
	widgetHandler.actionHandler:RemoveAction(self, "blueprint_set_hotkey", "p")
